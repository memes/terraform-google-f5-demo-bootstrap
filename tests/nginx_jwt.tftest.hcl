# Validate the module resources and outputs when nginx_jwt variables are modified from default.
#
...
variables {
  name       = "var-nginx-jwt-test"
  project_id = "mock-project-id"
}

# Setting nginx_jwt variable to empty string should result in same outputs as default value.
run "empty" {
  variables {
    nginx_jwt = ""
  }
  ...
  assert {
    condition     = google_artifact_registry_repository.automation["oci"].repository_id == "var-nginx-jwt-test-oci" && google_artifact_registry_repository.automation["oci"].format == "DOCKER" && google_artifact_registry_repository.automation["oci"].location == "us"
  ...
  assert {
    ...
    error_message = "Expected registries output to one entry."
  }
  assert {
  ...
  assert {
    condition     = alltrue([for k, v in google_secret_manager_secret.nginx_jwt : v.secret_id == "var-nginx-jwt-test-nginx-jwt"])
    error_message = "Expected NGINX JWT secret name to be 'var-nginx-jwt-test-nginx-jwt'."
  }
  assert {
  ...
  assert {
    condition     = alltrue([for k, v in google_secret_manager_secret.upstream_oci_password_nginx : v.secret_id == "var-nginx-jwt-test-upstream-oci-nginx"])
    error_message = "Expected upstream NGINX Docker secret name to be 'var-nginx-jwt-test-upstream-oci-nginx'."
  }
  assert {
  ...
  assert {
    condition     = alltrue([for k, v in google_secret_manager_secret.upstream_oci_password_f5_ai : v.secret_id == "var-nginx-jwt-test-upstream-oci-password-f5-ai"])
    error_message = "Expected upstream F5 AI harbor secret name to be 'var-nginx-jwt-test-upstream-oci-password-f5-ai'."
  }
  assert {
  ...
  assert {
    condition     = alltrue([for k, v in google_secret_manager_secret.secrets : startswith(v.secret_id, "var-nginx-jwt-test-")])
    error_message = "Expected additional secret names to start with 'var-nginx-jwt-test-'."
  }
}
...
run "valid" {
  ...
  assert {
    ...
    error_message = "Expected GitHub variable named 'OCI_NGINX_REGISTRY' for upstream NGINX OCI AR repo."
  }
  ...
  assert {
    ...
    error_message = "Expected one NGINX JWT secret to be created."
  }
  assert {
    condition     = alltrue([for k, v in google_secret_manager_secret.nginx_jwt : v.secret_id == "var-nginx-jwt-test-nginx-jwt"])
    error_message = "Expected NGINX JWT secret name to be 'var-nginx-jwt-test-nginx-jwt'."
